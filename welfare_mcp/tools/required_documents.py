    ...
    async with sem:
        try:
            ...
            def parse_docs(value: str | None) -> List[str]:
                if not value or value.strip() in ("", "-", "해당없음", "없음", "null", "NULL"): return []
                # 줄바꿈, 쉼표, 세미콜론 등으로 분리
            ...
            # [수정됨] 정확한 키 값 사용
            verified_by_officer = parse_docs(row["personal_verification_required_documents"]) + parse_docs(row["official_required_documents"])
            ...
            return {
                ...
                "verified_by_officer": list(dict.fromkeys(verified_by_officer)), # 중복 제거
